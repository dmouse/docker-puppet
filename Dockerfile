...
RUN source /etc/profile.d/rvm.sh && /usr/local/rvm/bin/rvm use 2.1.0 --default
RUN yum -y --nogpgcheck install "http://yum.puppetlabs.com/el/6/dependencies/x86_64/ruby-rgen-0.6.5-2.el6.noarch.rpm" >/dev/null
RUN yum -y --nogpgcheck install "http://yum.puppetlabs.com/el/6/products/x86_64/puppet-3.4.3-1.el6.noarch.rpm" >/dev/null
